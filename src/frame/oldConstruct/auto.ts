...
export function updateConstruction(room: Room): void {
    ...
    for (const myStructure of structures) {
        if (myStructure.hitsMax) {
            ...
            if (structureList && structureData && !structureData.memory[myStructure.id] && structureData.type) {
                structureData.memory[myStructure.id] = {
                    built: true,
                    pos: PosStr.setPosToStr(myStructure.pos),
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
                    id: myStructure.id as any
                };
            ...
            const structureList = myStructure.room.memory.construct.construction[myStructure.structureType];
            const structureData = structureList?.[myStructure.structureType];
            if (structureList && structureData && !structureData.memory[myStructure.id] && structureData.type) {
                structureData.memory[myStructure.id] = {
                    pos: PosStr.setPosToStr(myStructure.pos),
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
